...
class InnerChatActivity : BaseKotlinActivity<ActivityInnerChatBinding,InnerChatViewModel>(),SocketServiceManager{
    ...
    override fun onResume() {
        ...
        viewModel.makeChatRoom(myID,roomID,fixedPeopleList)
    ...
    override fun initDataBinding() {
         ...
         * */
        // makeChatRoom
        viewModel.cBaseResponseLiveData.observe(this, Observer { res->
            if (res.success){

                //charRoomId
                res.ChatRoomIdResponse.let {
                    Log.e(TAG,"채팅방 id : $it")
                    roomID = it
                ...
                //툴바제목
                res.ChatRoomNameResponse.let {
                    Log.e(TAG,"채팅방 이름 : $it")
                    setToolbarTitle(it)
                }

                //fixedPeopleList
                res.ChatRoomPeopleResponse.let {
                ...
                //drawer속 list view
                res.ChatRoomPeopleInfoListResponse.let {
                    peopleInfoList = it
    ...
    override fun onBackPressed() {
        ...
        when(drawer_layout.isDrawerOpen(GravityCompat.END)){
            true->{ drawer_layout.closeDrawer(GravityCompat.END) }
    ...
    private fun sendMessage(){
        ...
        if (!chat_edit_text.text.toString().isBlank()){
            ...
            roomID?.let { roomID ->
                ...
                ChatSocketData(roomID,myID,chat_edit_text.text.toString(),
    ...
    private fun disableSendButton(){ isOkToSend = false }

    private fun enableSendButton(){ isOkToSend = true }
}
